...
// Ambil data album
$query_album = mysqli_query($conn, "SELECT * FROM galeri WHERE id_kegiatan = $id_kegiatan");
...
<body>
    <header>
        ...
        <nav>
            ...
            <a href="https://sibooky.semarangkota.go.id/">PERPUSTAKAAN</a>
...
<body>
    ...
    <!-- Header -->
    <div class="container">
        <h2 class="judul-section">Album: <?= htmlspecialchars($data_album['nama_kegiatan']) ?></h2>
        <p class="text-center"><?= nl2br(htmlspecialchars($data_album['keterangan'])) ?></p>
    </div>
    ...
    <div class="container">
        <div class="row">
            <?php
            $query_isi = mysqli_query($conn, "SELECT * FROM media_galeri WHERE id_kegiatan = $id_kegiatan");
            if (mysqli_num_rows($query_isi) > 0) {
                while ($isi = mysqli_fetch_assoc($query_isi)) {
                    if ($isi['tipe'] == 'foto') {
                        echo '
                        <div class="col-md-4 galeri-item">
                            <img src="../image/' . htmlspecialchars($isi['nama_file']) . '" class="galeri-img" alt="">
                        </div>';
                    } elseif ($isi['tipe'] == 'video') {
                        echo '
                        <div class="col-md-6 galeri-item">
                            <video controls class="galeri-video">
                                <source src="../video/' . htmlspecialchars($isi['nama_file']) . '" type="video/mp4">
                ...
                echo '<p class="text-center">Belum ada foto atau video untuk kegiatan ini.</p>';
            }
            ?>
        </div>
    </div>
