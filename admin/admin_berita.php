...
<div class="container">
  <h3 class="text-center judul-section mt-4 py-2 rounded">Daftar Berita SDN Ngadirgo 02</h3>

  <div class="text-end mb-3">
    <a href="tambah_berita.php" class="btn btn-primary mt-4">Tambah</a>
  </div>
    ...
    <tbody>
      ...
      ?>
        <tr>
          ...
          <td><?= $data['judul'] ?></td>
          <td>
            <img src="../image/<?= $data['foto_berita'] ?>" style="width: 90px;">
          </td>
          <td><?= $data['isi'] ?></td>
          <td>
            <a href="edit_berita.php?id=<?= $data['id_berita']; ?>" class="btn btn-warning">Edit</a>
            <a href="delete_berita.php?id=<?= $data['id_berita']; ?>" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus?')">Delete</a>
          </td>
        </tr>
      <?php } ?>
    </tbody>
